...
// ── 측면 모드 (베타) — 상반신 3점(가까운 귀·어깨·코)만 사용 ──
// 폰을 옆에 두는 배치. 거북목(FHP)이 화면 X축에서 직접 보여 깊이 추정이 필요 없다.
// 임상 정의 그대로: FHP = 귀가 어깨보다 앞으로 나간 정도. 정규화는 귀-어깨 거리(목 길이).
export const TUNE_SIDE = {
  FHP_MARGIN: 0.15,      // 기준 대비 목길이의 15% 이상 귀가 앞으로 나가야 감점
  FHP_FULL: 0.25,
  NECK_MARGIN_DEG: 8,    // 목 전방 기울기 기준 +8°부터
  NECK_FULL_DEG: 20,
  SPITCH_MARGIN_DEG: 30, // 측면 고개 숙임(책 보기 허용 넉넉)
  SPITCH_FULL_DEG: 45,
};

// 2D 정규화 랜드마크에서 측면 지표 추출. 가까운 쪽(visibility 높은 쪽) 귀·어깨만 사용,
// '앞' 방향은 코의 위치로 자동 감지(왼쪽/오른쪽 배치 모두 지원). 미검출이면 null.
export function extractSide(lms) {
  if (!lms) return null;
  const N = { NOSE: 0, EAR_L: 7, EAR_R: 8, SH_L: 11, SH_R: 12 };
  const vis = (i) => lms[i]?.visibility ?? 0;
  const useL = vis(N.EAR_L) + vis(N.SH_L) >= vis(N.EAR_R) + vis(N.SH_R);
  const ear = lms[useL ? N.EAR_L : N.EAR_R], sh = lms[useL ? N.SH_L : N.SH_R], nose = lms[N.NOSE];
  if (!ear || !sh || (ear.visibility ?? 1) < 0.5 || (sh.visibility ?? 1) < 0.5) return null;
  const neck = Math.hypot(ear.x - sh.x, ear.y - sh.y);
  if (neck < 1e-4) return null;
  const fwdSign = (nose && (nose.visibility ?? 1) >= 0.3 && Math.sign(nose.x - ear.x)) || 1;
  const fhp = ((ear.x - sh.x) * fwdSign) / neck;                                  // 귀 전방 이동(목길이 단위, 앞=+)
  const neckAngle = Math.atan2((ear.x - sh.x) * fwdSign, Math.max(sh.y - ear.y, 1e-6)) * 180 / Math.PI; // 수직 대비 전방 기울기(°)
  let headPitch = null;                                                            // 코-귀 선의 내림각(숙임=+)
  if (nose && (nose.visibility ?? 1) >= 0.5) {
    headPitch = Math.atan2(nose.y - ear.y, Math.abs(nose.x - ear.x) + 1e-6) * 180 / Math.PI;
  }
  return { fhp, neckAngle, headPitch, side: useL ? "left" : "right" };
}

export class SideSmoother {
  constructor() {
    this.f = {
      fhp: new OneEuro({ minCutoff: 0.6, beta: 0.01 }),
      neckAngle: new OneEuro({ minCutoff: 0.6, beta: 0.02 }),
      headPitch: new OneEuro({ minCutoff: 0.8, beta: 0.02 }),
    };
  }
  update(m, t) {
    if (!m) return null;
    const o = { side: m.side };
    o.fhp = this.f.fhp.filter(m.fhp, t);
    o.neckAngle = this.f.neckAngle.filter(m.neckAngle, t);
    if (Number.isFinite(m.headPitch)) o.headPitch = this.f.headPitch.filter(m.headPitch, t);
    return o;
  }
}

// 측면 캘리브 기준 — 중앙값(이상치에 강건). headPitch는 표본 30% 이상일 때만.
export function finishSideRef(samples) {
  if (!samples || samples.length < 5) return null;
  const med = (k) => median(samples.map((s) => s[k]).filter(Number.isFinite));
  const ref = { fhp: med("fhp"), neckAngle: med("neckAngle") };
  const ps = samples.map((s) => s.headPitch).filter(Number.isFinite);
  if (ps.length >= Math.max(3, Math.floor(samples.length * 0.3))) ref.headPitch = median(ps);
  return ref;
}

function sideContribs(m, ref) {
  const S = TUNE_SIDE;
  const T = (x, mgn, full) => (x - mgn > 0 ? Math.min((x - mgn) / full, 1.5) : 0);
  const out = [
    ["head_drop", 1.3 * T(m.fhp - ref.fhp, S.FHP_MARGIN, S.FHP_FULL)],
    ["head_drop", 1.0 * T(m.neckAngle - ref.neckAngle, S.NECK_MARGIN_DEG, S.NECK_FULL_DEG)],
  ];
  if (Number.isFinite(m.headPitch) && Number.isFinite(ref.headPitch)) {
    out.push(["pitch", 0.6 * T(m.headPitch - ref.headPitch, S.SPITCH_MARGIN_DEG, S.SPITCH_FULL_DEG)]);
  }
  return out;
}
// 정면 absPenalty와 동일한 결합(지배 신호 full + 나머지 0.35 할인)
export function sidePenalty(m, ref) {
  if (!m || !ref || !Number.isFinite(ref.fhp)) return 0;
  const cs = sideContribs(m, ref).map(([, c]) => c);
  const sum = cs.reduce((a, b) => a + b, 0), max = cs.reduce((a, b) => Math.max(a, b), 0);
  return max + 0.35 * (sum - max);
}
export function sideDominant(m, ref) {
  if (!m || !ref || !Number.isFinite(ref.fhp)) return null;
  let bestKey = null, bestC = 0;
  for (const [k, c] of sideContribs(m, ref)) if (c > bestC) { bestC = c; bestKey = k; }
  return bestKey ? { key: bestKey, c: bestC } : null;
}
