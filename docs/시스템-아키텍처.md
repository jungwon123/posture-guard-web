...
### CI/CD — GitHub Actions (2026-07-23 구축)

저장소: `github.com/jungwon123/posture-guard` (private, 기본 브랜치 `realtime-camera`)

| 워크플로 | 트리거 | 하는 일 |
|---|---|---|
| CI | 모든 push/PR | 프론트 유닛 테스트 45개 + 프로덕션 빌드, 백엔드 문법·의존성 검증 |
| Deploy backend | `backend-vm/**`·`infra/**` push | rsync로 VM 동기화(.env 보존) → `docker compose build && up` → 내부·외부 헬스체크 |
| Deploy frontend | `frontend/**` push | Vercel 프로덕션 배포 (`VERCEL_TOKEN` 시크릿 없으면 자동 건너뜀) |

- 배포 인증: 전용 SSH 키(GitHub 시크릿 `DEPLOY_SSH_KEY` ↔ VM authorized_keys) — gcloud 인증과 독립.
- 아래 수동 배포 절차는 **CI/CD 장애 시 폴백**으로 유지.
