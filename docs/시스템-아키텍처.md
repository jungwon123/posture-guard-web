...
flowchart LR
  ...
  subgraph GCP["GCP Compute Engine VM — pg-api-vm · Docker Compose 스택 (asia-northeast3-a, e2-small)"]
    NGINX["nginx 게이트웨이 (컨테이너)<br/>TLS 종료(Let's Encrypt)·rate limit<br/>api: 34-64-158-222.sslip.io<br/>sfu: lk.34-64-158-222.sslip.io"]
    API["Express API (컨테이너 api)<br/>compose 내부망 :8080<br/>외부 노출은 nginx뿐"]
    DB[("PostgreSQL 16<br/>(컨테이너, pgdata 볼륨)")]
    REDIS[("Redis 7 (컨테이너, 64MB LRU)<br/>presence TTL 90s<br/>리더보드 캐시 30s")]
    LKS["livekit-server v1.13.4<br/>(컨테이너, host 네트워크 — SFU 자체 운영)<br/>:7880 ws · 미디어 udp 50000-50200 · tcp 7881 폴백"]
    NGINX --> API
    NGINX -- "wss 시그널링" --> LKS
    API -- "pg 풀 (max 10)" --> DB
...
- 환경 변수: VM의 `~/stack/infra/.env` (git 제외, VM에만 존재) — `POSTGRES_PASSWORD`(마이그레이션 때 신규 생성·로테이트), `LIVEKIT_URL/API_KEY/API_SECRET`(미설정 시 rtc-token 503), `LIVEKIT_SELF_*`(셀프호스트 옵트인 + livekit 컨테이너 키), `GOOGLE_CLIENT_ID`(미설정 시 google-login 503).
- 인증서: certbot 컨테이너로 발급·갱신(월 1회 `renew` — infra/README.md), nginx가 서빙.
- 데이터: `pgdata` 도커 볼륨. 백업은 `docker compose exec -T db pg_dump -U pguser posture > 파일` — VM 삭제 전 필수는 동일.

### 셀프호스트 SFU (livekit-server) 운영 요약

- compose 서비스 `livekit`(프로필 `vm`) — 공식 이미지 v1.13.4, **host 네트워크**(UDP 대역 성능), 설정 `infra/livekit/livekit.yaml`(비밀키 없음 — 키는 `.env`의 `LIVEKIT_SELF_KEY/SECRET`를 compose가 `LIVEKIT_KEYS`로 주입).
- nginx 사이트 `lk.34-64-158-222.sslip.io` → host의 :7880 (wss), GCP 방화벽 룰 `livekit-rtc`(tcp:7881, udp:50000-50200)는 그대로.
- 전환: 클라이언트 `?lkself=1`(켬)/`?lkself=0`(끔) — 서버 기본값은 항상 Cloud. 끄기: `sudo docker compose stop livekit`(Cloud 경로 무영향).
